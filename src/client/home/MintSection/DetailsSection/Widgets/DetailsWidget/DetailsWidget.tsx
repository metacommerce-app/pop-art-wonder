    ...
    <div className={`my-4 border border-gray-300 p-4 rounded-md ${className}`}>
      ...
      <div className='grid grid-cols-2 gap-4 mt-4'>
        <div className='flex flex-col gap-4'>
          <DetailItem title={t("Opensea URL")} value={clientConfig.openseaUrl} className='!lowercase' />
          <DetailItem title={t('Token Standard')} value={t('ERC721')} />
          <DetailItem title={t('Start Date')} value={dayjs(clientConfig.startDate).format('YYYY-MM-DD')} />
        </div>
        <div className='flex flex-col gap-4'>
          <DetailItem title={t("Contract Address")} value={toParsedAddress(clientConfig.contractAddress)} />
          <DetailItem title={t("Blockchain")} value={t("Ethereum")} />
          <DetailItem title={t('End Date')} value={dayjs(clientConfig.endDate).format('YYYY-MM-DD')} />
        </div>
      </div>
    </div>
  );
